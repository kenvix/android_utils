...
public class BaseActivityUI {
    ...
    public void showExceptionToastPrompt(Throwable throwable) {
        showToast(context.getString(ApplicationEnvironment.getAppResourceIdentifier("error_operation_failed", "string")) + throwable.getLocalizedMessage());
    ...
    public void showExceptionSnackbarPrompt(Throwable throwable) {
        showSnackbar(context.getString(ApplicationEnvironment.getAppResourceIdentifier("error_operation_failed", "string")) + throwable.getLocalizedMessage(), Snackbar.LENGTH_LONG);
        LogUtilsKt.severe(context.getLogger(), throwable, "Global Exception Prompt: Operation FAILED");
    }
    ...
    public static AlertDialog.Builder getAlertBuilder(Context context, String text, @Nullable String title, @Nullable Consumer<Boolean> callback) {
        AlertDialog.Builder builder = new AlertDialog.Builder(context)
                ...
                .setPositiveButton(context.getString(ApplicationEnvironment.getAppResourceIdentifier("action_ok", "string")), (dialog, which) -> {
                    if (callback != null)
    ...
    public AlertDialog.Builder getConfirmBuilder(String text, @Nullable String title, @Nullable Consumer<Boolean> callback) {
        return getAlertBuilder(text, title, callback)
                .setNegativeButton(context.getString(ApplicationEnvironment.getAppResourceIdentifier("action_cancel", "string")),(dialog, which) -> {
                    if(callback != null)
                        callback.accept(false);
                });
    }
    ...
    public ProgressDialog.Builder getProgressDialogBuilder(String text, boolean isCancelable, @Nullable String title, @Nullable Consumer<Boolean> callback) {
        ...
        if (isCancelable) {
            builder.setNegativeButton(context.getString(ApplicationEnvironment.getAppResourceIdentifier("action_cancel", "string")),(dialog, which) -> {
                if(callback != null)
                    callback.accept(false);
            });
        }
